...
<div class="eventorganiser-event-meta">
	...
	<!-- Event details -->
	<h4><?php _e( 'Performance', 'eventorganiser' ); ?></h4>
	...
	<ul class="eo-event-meta">
		...
		<?php if ( eo_recurs() ) {
				...
				if ( $upcoming->have_posts() ) : ?>
					...
					<li><strong><?php _e( 'Dates', 'eventorganiser' ); ?>:</strong>
		...
		<?php if ( get_the_terms( get_the_ID(), 'event-category' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-category' ) ) ) { ?>
			<li> <?php echo get_the_term_list( get_the_ID(),'event-category', '', ', ', '' ); ?> <?php esc_html_e( 'Performance', 'eventorganiser' ); ?></li>
		<?php } ?>
